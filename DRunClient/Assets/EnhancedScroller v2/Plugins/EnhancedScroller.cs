...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        // 2022.04.26 소현 : 아무튼 이게 true 면 스냅한다!
        private bool _isDragged = false;

        // 2022.04.27 소현 : 이 스피드 이상의 속도로 스크롤하면 스냅됨
        [SerializeField]
        private float snapableSpeed = 120f;
        ...
        // 2022.04.26 소현 : 빨리 스크롤 할 때 스냅될 수 있도록!
        public void Snap(float prevScrollPosition)
        {
            ...
            // 이전 위치와 비교할 거야!
            int prevCellViewIndex = GetCellViewIndexAtPosition(prevScrollPosition);

            /*            Debug.LogFormat("prev : {0} / curr : {1}", prevCellViewIndex, _snapCellViewIndex);
                        Debug.LogFormat("linear velocity : {0}", LinearVelocity);*/

            // 넘어가지 않았지만 속도가 충분히 빠른 경우
            if (LinearVelocity > snapableSpeed && prevCellViewIndex + 1 == _snapCellViewIndex)
            {
                _snapCellViewIndex -= 1;
            }
            else if (LinearVelocity < -snapableSpeed && prevCellViewIndex == _snapCellViewIndex)
            {
                _snapCellViewIndex += 1;
            }
        ...
        // 2022.04.27 소현 : 스크콜러의 양 극단에서 스냅
        public void Snap(int endIndex)
        {
            if (NumberOfCells == 0) return;
        ...
        public void OnBeginDrag(PointerEventData data)
        {
            ...
            // 2022.04.26 소현 : 이걸로 snap 들어갈지 말지 결정
            _isDragged = true;
        }
        ...
        public void OnEndDrag(PointerEventData data)
        {
            ...
            // 2022.04.26 소현 : 멈췄는데 아직 onvaluechanged 에서 스냅되지 않았다면
            if (snapping && !_snapJumping && _isDragged && LinearVelocity < 1f && LinearVelocity > -1f)
            {
                _isDragged = false;
                Snap();
            }
        ...
        {
            float prevScrollPosition = _scrollPosition;
            ...
            if (snapping && !_snapJumping)
            {
                ...
                // 2022.04.26 소현 : 속도 말고 드래그 여부로 스냅
                if (Mathf.Abs(LinearVelocity) <= snapVelocityThreshold && _isDragged /*&& LinearVelocity != 0*/)
                {
                    _isDragged = false;
                    ...
                    if (loop || (!loop && normalized > 0 && normalized < 1.0f))
                    {
                        ...
                        Snap(prevScrollPosition);
                    }
                    else if (!loop && normalized <= 0)
                    {
                        Snap(0);
                    }
                    else if (!loop && normalized >= 1f)
                    {
                        Snap(NumberOfCells - 1);
                    }
                }
            }
