...
namespace Festa.Client
{
    ...
    public class LifeStatGraphRenderer : LineSegmentsRenderer
    {
        [SerializeField]
        ...
        public void onClickVertex(int vertexIndex)
        {
            Vector2 targetPos = targetPoints[vertexIndex];
            targetPos.y += _labelOffsetY;
            rect_label.anchoredPosition = targetPos;
            txt_label.text = targetLength[vertexIndex].ToString();
            rect_label.gameObject.SetActive(true);
            Invoke("deactivateLabel", 0.8f);
        }
