...
namespace Festa.Client.MapBox
{
    ...
    public class AABBTree
    {
        ...
        private List<Collider> _collidedColliderList = new List<Collider>();    // 진짜로 충돌
        ...
        private void insertLeaf(AABBNode leafNode)
        {
            ...
            while (!rootNode.isLeaf())
            {
                ...
                // 여기서 새로운 parent node 를 만드느 게 더 비용이 싸다면!! 여기다가 새로운 부모를 만들고 leaf 를 붙인당
...
/*        public void removeSymbolRenderer(UMBSymbolRenderer renderer)
        {
            removeLeaf(renderer.AABBNode);
            deallocateNode(renderer.AABBNode);
        }*/
...
/*                if (node.left == null)
        ...
        public void queryOverlaps(Collider collider,List<Collider> list)
        {
            ...
            //Debug.Log($"test counts : {testCount}");
        }
        ...
        private void deallocateNode(AABBNode node)
        {
        }

        // for debugging purpose
/*        public void getNodeInfo(int nodeID, int left, int right, AABB aabb)
        {
        }*/
    }
}