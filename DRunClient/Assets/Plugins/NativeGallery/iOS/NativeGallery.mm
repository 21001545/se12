...
typedef void (*thumbnailPathDelegate)(const char* path, const char* uniqueId);
...
+ (char *)getRecentImagePaths:(int)maximumSize count:(int)count;
+ (char *)getImageThumbnailPathFromImagePath:(NSString *)imagePath with:(int)width height:(int)height saveFolderPath:(NSString *)saveFolderPath needOverride:(bool)needOverride;
@end
...
+ (char *)getRecentImagePaths:(int)startIndex count:(int)count
{
    NSLog( @"run getRecentImagePaths" );
    PHFetchOptions *fetchOption = [[PHFetchOptions alloc] init];
    // 이게..없네... limit만 있고, start가 없네...
    // startIndex + count까지 fetch 해오고, startIndex부터 순회하면서 가져와보자.
    fetchOption.fetchLimit = startIndex + count;
    fetchOption.sortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:@"creationDate" ascending: false]];
    PHFetchResult* fetchPhotos = [PHAsset fetchAssetsWithMediaType:PHAssetMediaTypeImage options:fetchOption];
    
    int fetchCount = fetchPhotos.count - startIndex;
    if ( fetchCount <= 0 )
    {
        NSLog( @"fetch result count is zero..." );
        return nil;
    }
    
    NSLog( @"fetch result count is %d", fetchCount );
    NSMutableArray<NSString *> *resultPaths = [NSMutableArray arrayWithCapacity:fetchCount];
    //NSLock *arrayLock = [[NSLock alloc] init];
    ...
    bool ret = false;
    for( int i = startIndex; i < fetchPhotos.count; i++ )
    {
        PHAsset* asset = [fetchPhotos objectAtIndex:i];
        ...
        if (  ret == false )
        {
            uint count;
            NSLog(@"PropertyCheck");
            objc_property_t *classPropertiesArray = class_copyPropertyList([asset class],&count);
            for ( int k = 0 ; k < count;++k)
            {
                const char* name = property_getName(classPropertiesArray[k]);
                NSLog(@"%s", name);
            }
            free(classPropertiesArray);
            ret = true;
            
            NSLog(@"PropertyCheck Done");
        }
        
        [resultPaths addObject:@""];
        [resultPaths addObject:asset.localIdentifier];
    }
    
    return [self getCString:[resultPaths componentsJoinedByString:@">"]];
}

+ (void)getImageThumbnailPathFromIdentifyAsync:(NSString *)identify with:(int)width height:(int)height saveFolderPath:(NSString *)saveFolderPath thumbnailPathCallback:(thumbnailPathDelegate)thumbnailPathCallback uniqueId:(NSString*)uniqueId needOverride:(bool)needOverride
{
    ...
    PHFetchResult* fetchResult = [PHAsset fetchAssetsWithLocalIdentifiers:[NSArray arrayWithObject:(NSString*)identify] options:fetchOption];
    if ( fetchResult == nil || fetchResult.count == 0 )
    {
        // ..? 어떻하지?
        NSLog( @"fetchResult nil or zero");
        dispatch_async(queue, ^{
            thumbnailPathCallback([self getCString:nil], [self getCString:nil]);
        });
        return;
    }
	...
	if( width == -1 && height == -1)
	{
        ...
        if( width > 1440 || height > 1440)
        {
            ...
            {
                newHeight = 1440;
                newWidth = 1440 * width / height;
            }
            else
            {
                newWidth = 1440;
                newHeight = 1440 * height / width;
            }
            
            NSLog(@"resize image: [%dx%d] -> [%dx%d]", width, height, newWidth, newHeight);
    ...
    NSString* savePath = [NSString stringWithFormat:@"%@_%d_%d_%@.jpg", saveFolderPath, width, height, [[[asset valueForKey:@"pathForOriginalImageFile"] lastPathComponent] stringByDeletingPathExtension] ];
    
    if ( [[NSFileManager defaultManager] fileExistsAtPath:savePath] == NO || needOverride == YES)
    {
        PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
        options.synchronous = NO;
        options.networkAccessAllowed = YES;
		// 때때로 매우 작은 이미지가 로딩되는 경우가 있다
		options.deliveryMode = PHImageRequestOptionsDeliveryModeHighQualityFormat;
        
        // icloude데이터도 가져오려면 어쩔수 없이 phasset을 통하긴 해야함.
        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:targetSize contentMode:PHImageContentModeDefault options:options resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            // 2022.03.10 이강희 jpg화일을 75퍼센트로 조절
            [UIImageJPEGRepresentation(result, 0.75f) writeToFile:savePath atomically:NO];
            dispatch_async(queue, ^{
                thumbnailPathCallback([self getCString:savePath], [self getCString:uniqueId]);
            });
        }];
    }
    else
...
+ (void)getImageThumbnailPathFromImagePathAsync:(NSString *)imagePath with:(int)width height:(int)height saveFolderPath:(NSString *)saveFolderPath thumbnailPathCallback:(thumbnailPathDelegate)thumbnailPathCallback uniqueId:(NSString*)uniqueId needOverride:(bool)needOverride
{
    NSLog( @"run getRecentImagePaths" );
    PHFetchOptions *fetchOption = [[PHFetchOptions alloc] init];
    // 이게..없네... limit만 있고, start가 없네...
    ...
    int fetchCount = fetchPhotos.count;
    for( int i = 0; i < fetchCount; i++ )
    {
        PHAsset* asset = [fetchPhotos objectAtIndex:i];
        PHFetchResult* localFetchResult = [PHAsset fetchAssetsWithLocalIdentifiers:[NSArray arrayWithObject:(NSString*)asset.localIdentifier] options:fetchOption];
        if ( localFetchResult == nil)
            continue;
        if ( localFetchResult.count > 0 )
        {
            PHAsset* localAsset = [localFetchResult objectAtIndex:0];
            if ( [[localAsset valueForKey:@"pathForOriginalImageFile"] isEqualToString:[asset valueForKey:@"pathForOriginalImageFile"]] == NO )
            {
                NSLog(@"%@ %@", [asset valueForKey:@"pathForOriginalImageFile"], [localAsset valueForKey:@"pathForOriginalImageFile"]);
            }
        }
    }
    /*
    ...
    */
    ...
    {
    NSLog(@"%@", imageInfo);
    NSString *filePath = imageInfo[@"PHImageFileURLKey"];
    if( filePath != nil ) // filePath can actually be an NSURL, convert it to NSString
    filePath = [NSString stringWithFormat:@"%@", filePath];
    
    if( filePath == nil || [filePath length] == 0 )
    {
    filePath = imageInfo[@"PHImageFileUTIKey"];
    if( filePath != nil )
    filePath = [NSString stringWithFormat:@"%@", filePath];
    }
    ...
    {
    //...없다구..?
    NSLog( @"Couldn't find image path" );
    }
    else
    {
    [arrayLock lock];
    [resultPaths addObject:filePath];
    [arrayLock unlock];
    }
    }
    }];*/
...
+ (char *)getImageThumbnailPathFromImagePath:(NSString *)imagePath with:(int)width height:(int)height saveFolderPath:(NSString *)saveFolderPath needOverride:(bool)needOverride
{
    // PhotoKit에선 file:// 형식의 URL로 특정 asset을 찾을 수 없음
    // ImageIO framework를 이용하여 네이티브로 구현해야할 듯
    NSString* savePath = [NSString stringWithFormat:@"%@_%d_%d_%@", saveFolderPath, width, height, [imagePath lastPathComponent]];
    if ( [[NSFileManager defaultManager] fileExistsAtPath:savePath] == NO || needOverride == YES)
    {
        ...
        if( ![UIImageJPEGRepresentation(scaled, 100.0f) writeToFile:savePath atomically:YES] )
        {
            NSLog( @"Error saving thumbnail image" );
        }
    }
...
extern "C" char* _NativeGallery_GetRecentImagePaths(int startIndex, int count)
{
	return [UNativeGallery getRecentImagePaths:startIndex count:count];
}

extern "C" char* _NativeGallery_GetImageMiniThumbnailPathFromImagePath(const char* imagePath, int width, int height, const char* saveFolderPath, bool needOverride )
{
    return [UNativeGallery getImageThumbnailPathFromImagePath:[NSString stringWithUTF8String:imagePath] with:width height:height saveFolderPath:[NSString stringWithUTF8String:saveFolderPath] needOverride:needOverride];
}


extern "C" void _NativeGallery_GetImageThumbnailPathFromImagePathAsync(const char* imagePath, int width, int height, const char* saveFolderPath, thumbnailPathDelegate callback, const char* uniqueId, bool needOverride )
{
    [UNativeGallery getImageThumbnailPathFromImagePathAsync:[NSString stringWithUTF8String:imagePath] with:width height:height saveFolderPath:[NSString stringWithUTF8String:saveFolderPath] thumbnailPathCallback:callback uniqueId:[NSString stringWithUTF8String:uniqueId] needOverride:needOverride];
}

extern "C" void _NativeGallery_GetImageThumbnailPathFromIdentifyAsync(const char* identify, int width, int height, const char* saveFolderPath, thumbnailPathDelegate callback, const char* uniqueId, bool needOverride )
{
    [UNativeGallery getImageThumbnailPathFromIdentifyAsync:[NSString stringWithUTF8String:identify] with:width height:height saveFolderPath:[NSString stringWithUTF8String:saveFolderPath] thumbnailPathCallback:callback uniqueId:[NSString stringWithUTF8String:uniqueId] needOverride:needOverride];
}
