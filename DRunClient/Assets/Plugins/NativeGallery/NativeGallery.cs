...
public static class NativeGallery
{
    ...
    public class NativePhotoContext
    {
        public string path;
        public string identifier;

		public bool justifiedHEIC;

        public NativePhotoContext() { }
        public NativePhotoContext(string path)
        {
            this.path = path;
            this.identifier = path;
			this.justifiedHEIC = false;
        }

        public NativePhotoContext(string path, string identifier)
        {
            this.path = path;
		...
		public NativePhotoContext(string path, string identifier,bool justifiedHEIC)
        {
			this.path = path;
			this.identifier = identifier;
			this.justifiedHEIC = justifiedHEIC;
        }

		public override bool Equals(object obj)
		{
			NativePhotoContext ctx = obj as NativePhotoContext;
			if( ctx == null)
			{
				return false;
			}
	...
	public class ThumbnailPathCallbackAndroid : AndroidJavaProxy
    {
		private ThumbnailPathCallback callback;
		private string uniqueId;

		public ThumbnailPathCallbackAndroid(ThumbnailPathCallback callback, string uniqueId ) : base("com.yasirkula.unity.ThumbnailPathReceiver")
		{
			this.callback = callback;
			this.uniqueId = uniqueId;
		}

		public void OnThumbnailPath(string path, string identifier)
        {
			callback?.Invoke(path, identifier);
        }
	}
#endif
	...
	// path : Thumbnail path of original image
	// uniqueId : Unique identify of async job 
	public delegate void ThumbnailPathCallback(string path, string uniqueId);
	private static Dictionary<string, ThumbnailPathCallback> CacheGetThumbnalCallbacks = new Dictionary<string, ThumbnailPathCallback>();
	...
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern string _NativeGallery_GetRecentImagePaths( int startIndex, int count );
	
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern string _NativeGallery_GetImageMiniThumbnailPathFromImagePath( string imagePath, int width, int height, string saveFolderPath, bool needOverride );

	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_GetImageThumbnailPathFromImagePathAsync( string imagePath, int width, int height, string saveFolderPath, ThumbnailPathCallback callback, string uniqueId, bool needOverride );

	[System.Runtime.InteropServices.DllImport( "__Internal" )]
	private static extern void _NativeGallery_GetImageThumbnailPathFromIdentifyAsync( string imagePath, int width, int height, string saveFolderPath, ThumbnailPathCallback callback, string uniqueId, bool needOverride );
#endif
	...
	public static bool Permitted(bool redirectToPermissionSettingsAfterDeny = false)
	{
		var permission = CheckPermission(PermissionType.Write);
		if (permission == Permission.ShouldAsk)
		{
			var permissionRequest = RequestPermission(PermissionType.Write);
			if (permissionRequest == Permission.Denied)
			{
				Debug.Log("[Share][NativeGallery] 쓰기 권한 요청 거절됨!");

				if (redirectToPermissionSettingsAfterDeny)
				{
					// TODO: 권한 설정창 이동.
	...
	private const bool PermissionFreeMode = false;
...
#region Load Functions
	...
	public static string GetImageMiniThumbnailPathFromImagePath(string imagePath, int width, int height, bool needOverride = false)
    {
        Permission permission = RequestPermission(PermissionType.Read);
        ...
        if (permission == Permission.Granted)
        {
			...
			return imagePath;
#elif UNITY_ANDROID
			result = AJC.CallStatic<string>("GetImageMiniThumbnailPathFromImagePath", Context, imagePath, TemporaryImagePath, width, height);
#elif UNITY_IOS
			result = _NativeGallery_GetImageMiniThumbnailPathFromImagePath(imagePath, width, height, TemporaryImagePath, needOverride);
#endif
		}
	...
	[MonoPInvokeCallback(typeof(ThumbnailPathCallback))]
	public static void OnThumbnailPathDelegate(string path, string uniqueId)
	{
			lock(CacheGetThumbnalCallbacks)
			{
				if ( CacheGetThumbnalCallbacks.ContainsKey(uniqueId))
				{	
					CacheGetThumbnalCallbacks[uniqueId]?.Invoke(path, uniqueId);
					CacheGetThumbnalCallbacks.Remove(uniqueId);
				}
			}
	}

	public static string GetImageThumbnailPathFromPhotoContextAsync(NativePhotoContext context, int width, int height, ThumbnailPathCallback callback, bool needOverride = false)
    {
        Permission permission = RequestPermission(PermissionType.Read);

        if (permission == Permission.Granted)
        {
			string uniqueId = (Time.time.ToString() + UnityEngine.Random.value.ToString());
			lock(CacheGetThumbnalCallbacks)
			{
				CacheGetThumbnalCallbacks.Add(uniqueId, callback);
			}

#if UNITY_EDITOR
			OnThumbnailPathDelegate(context.path, uniqueId);
#elif UNITY_ANDROID
			AJC.CallStatic("GetImageThumbnailPathFromImagePathAsync", Context, context.path, TemporaryImagePath, width, height, uniqueId, new ThumbnailPathCallbackAndroid( callback, uniqueId ) );
#elif UNITY_IOS
			_NativeGallery_GetImageThumbnailPathFromIdentifyAsync(context.identifier, width, height, TemporaryImagePath, OnThumbnailPathDelegate, uniqueId, needOverride);
#endif
			return uniqueId;
		}
	...
	public static List<NativePhotoContext> GetRecentImagePaths(int startIndex, int count)
    {
        Permission permission = RequestPermission(PermissionType.Read);
		
		List<NativePhotoContext> resultList  = null;
        if (permission == Permission.Granted )
        {
#if UNITY_EDITOR
			// 아무 이미지 여러개 막 줘야겠는걸?
#elif UNITY_ANDROID
			var obj = AJC.CallStatic<AndroidJavaObject>("GetRecentImagePaths", Context, startIndex, count);
			if (obj.GetRawObject().ToInt32() != 0)
			{
				var result = AndroidJNIHelper.ConvertFromJNIArray<string[]>(obj.GetRawObject());

				resultList = new List<NativePhotoContext>();
				resultList.Capacity = result.Length;
				for( int i = 0; i < result.Length; ++i)
				{
					resultList.Add(new NativePhotoContext(result[i]));
				}
			}
			obj.Dispose();
#elif UNITY_IOS
			...
			// 2021.12.01 이강희 갤러리에 이미지가 하나도 없을 경우 crash되는 이슈 해결
			string paths = _NativeGallery_GetRecentImagePaths( startIndex, count );				
			if( paths != null)
			{
				...
				{
					resultList.Add(new NativePhotoContext(result[i], result[i+1]));
				}
			}
#endif
		}
...
#region Utility Functions
		...
		bool generateMipmaps = true, bool linearColorSpace = false)
    {
		...
		try
		{
            ...
            {
                throw new Exception("LoadImage Fail!");
            }
        }
		catch( Exception e )
		{
