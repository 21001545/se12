...
namespace TMPro
{
    ...
    public abstract class TMP_Text : MaskableGraphic
    {
        ...
        // FESTA-20 swpark, iOS 이모지 개발
        public bool IsEmojiSupport
        {
            get { return m_IsEmojiSupport; }
            set {
                Debug.Log($"IsEmojiSupport :{m_IsEmojiSupport}->{value}");

                if (m_IsEmojiSupport == value) return; 
                // 재갱신하자.
                m_IsEmojiSupport = value; m_havePropertiesChanged = true; SetVerticesDirty(); SetLayoutDirty(); }
        }
        [SerializeField]
        protected bool m_IsEmojiSupport = true;
        ...
        protected float m_fontSizeBase = 20;
        ...
        void PopulateTextProcessingArray()
        {
            ...
            if (m_TextProcessingArray.Length < srcLength + writeIndex)
                ResizeInternalArray(ref m_TextProcessingArray, srcLength + writeIndex);
            tag_NoParsing = false;
            ...
            for (; readIndex < srcLength; readIndex++)
            {
                ...
                {
                    m_TextProcessingArray[writeIndex] = new TextProcessingElement { elementType = m_IsEmojiSupport ? TextProcessingElementType.TextEmojiElement : TextProcessingElementType.TextCharacterElement, stringIndex = readIndex, length = 2, unicode = TMP_TextParsingUtilities.ConvertToUTF32(c, m_TextBackingArray[readIndex + 1]) };
                    readIndex += 1;
                    writeIndex += 1;
                    continue;
                }
                ...
                m_TextProcessingArray[writeIndex] = new TextProcessingElement { elementType = TMP_TextParsingUtilities.IsEmoji(c) ? TextProcessingElementType.TextEmojiElement : TextProcessingElementType.TextCharacterElement, stringIndex = readIndex, length = 1, unicode = c };
        ...
        */
        ...
        protected virtual Vector2 CalculatePreferredValues(ref float fontSize, Vector2 marginSize, bool isTextAutoSizingEnabled, TextWrappingModes textWrapMode)
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                // FESTA-20 swpark, iOS 이모지 개발
                else if (m_TextProcessingArray[i].elementType == TextProcessingElementType.TextEmojiElement)
                {
        ...
        internal TMP_TextElement GetEmojiTextElement(string emoji)
        {
            var hash = TMP_TextUtilities.GetSimpleHashCode(emoji);
            return TMP_FontAssetUtilities.GetSpriteCharacterFromSpriteAsset((uint)hash, TMP_EmojiHelper.emojiSpriteAsset, true);
        }
        ...
        // FESTA-20 swpark, iOS 이모지 개발
        #region FESTA-20
        public static int GetIndexFirstEmoji(string text)
        {
        ...
        protected void GetRangeFirstEmoji(string text, int index, out int endIndex)
        {
            var utf16 = text;
            var utfIndex = index;

            long character = utf16[utfIndex]; // codepoint 확인용..

            if ((utf16[utfIndex] & 0xFC00) == 0xD800)
            {
                // high surrogate
                character = ((uint)((utf16[utfIndex] - 0xD800) * 0x400 + (utf16[utfIndex + 1] - 0xDC00) + 0x10000));
                ++utfIndex;
            }

            // modifier들을 검사하여.. 이모지 묶음을 찾아내자.
            int glyphByte = 2;
            int backupUtfIndex = utfIndex;
            bool joiner = false;

            while (utfIndex + 1 < utf16.Length)
            {
                if ((utf16[utfIndex + 1] & 0xFC00) == 0xD800)
                {
                    long nextCharacter = ((uint)((utf16[utfIndex + 1] - 0xD800) * 0x400 + (utf16[utfIndex + 2] - 0xDC00) + 0x10000));
                    if (joiner)
                    {
                        glyphByte += 2;
                        utfIndex += 2;
                    ...
                    else if (0x1F3FB <= nextCharacter && nextCharacter <= 0x1F3FF)
                    {
                        // color modifier
                        glyphByte += 2;
                        utfIndex += 2;
                ...
                else if (utfIndex + 2 < utf16.Length && utf16[utfIndex + 2] == 0xFE0F)
                {
                    // selector
                    glyphByte += 2;
                    utfIndex += 2;
                }
                else if (utf16[utfIndex + 1] >= 0x2600 && utf16[utfIndex + 1] <= 0x26FF)
                {
                    if ( joiner)
                    {
                        // 이어 붙여야하네?
                ...
                else if (utf16[utfIndex + 1] == 0xFE0F)
                {
                    // selector
                    ++glyphByte;
                    ++utfIndex;
                }
                else if (utf16[utfIndex + 1] == 0x200B)
                {
                    // zero width joiner..
                    ++glyphByte;
                    ++utfIndex;
                ...
                else if (utf16[utfIndex + 1] == 0x200D)
                {
                    // joiner
                    ++glyphByte;
                    ++utfIndex;
        ...
        protected void OnTextChanged(UnityEngine.Object obj)
        {
            if (obj != this || m_IsEmojiSupport == false)
            {
                return;
            }

            // 이모지를 갱신해볼까~
            // 일단 하나씩 가져와서 스프라이트시트에서 찾아본다음에 없으면,
            // 각 OS에서 렌더링 한다음?
            // 스프라이트 시트에 업데이트 하고~ 렌더링한다.

            var changedText = text;
            var emojiIndex = GetIndexFirstEmoji(changedText);
            while ( emojiIndex >= 0 )
            {
                int endIndex = emojiIndex;
                GetRangeFirstEmoji(changedText, emojiIndex, out endIndex);

                string emoji = changedText.Substring(emojiIndex, endIndex - emojiIndex + 1);
                if ( TMP_EmojiHelper.GetEmojiFromSpriteAsset(emoji, false) == false )
                {
                    // 갱신 필요.
                    StartCoroutine(ApplyChangesNextFrame());
                }

                changedText = changedText.Substring(endIndex + 1);
                emojiIndex = GetIndexFirstEmoji(changedText);
            }
        }
        ...
        internal void GetRangeEmoji(TextProcessingElement[] chars, int startIndex, out int endIndex)
        {
            // modifier들을 검사하여.. 이모지 묶음을 찾아내자.
            bool joiner = false;
            var index = startIndex;
            for ( ; index+ 1 < chars.Length && chars[index+1].unicode != 0 ;)
            {
                var nextCharacter = chars[index + 1].unicode;
                // 다음것도 이모지인데? 연결되는 이모지 일까?
                if (nextCharacter >= 0x2600 && nextCharacter <= 0x26FF)
                {
                    if (joiner)
                    {
                        // 이어 붙여야하네?
                ...
                else if (nextCharacter == 0x200B)
                {
                    // zero width joiner..
                    ++index;
                    joiner = true;
                }
                else if (nextCharacter == 0x200D)
                {
                    // joiner
                    ++index;
                    joiner = true;
                }
                else if (nextCharacter == 0xFE0F)
                {
                    // selector
                    ++index;
                }
                else if (TMP_TextParsingUtilities.IsEmoji(nextCharacter))
                {
                    if (joiner)
                    {
                        ++index;
                ...
                else if (index + 2 < chars.Length && chars[index + 2].unicode == 0xFE0F)
                {
                    // selectorr
                    index += 2;
                }
