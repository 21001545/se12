    ...
    public partial class TextMeshProUGUI
    {
        ...
        internal override int SetArraySizes(TextProcessingElement[] textProcessingArray)
        {
            ...
            */
            ...
            for (int i = 0; i < textProcessingArray.Length && textProcessingArray[i].unicode != 0; i++)
            {
                ...
                #endregion
                // FESTA-20 swpark, iOS 이모지 개발
                else if ( textProcessingArray[i].elementType == TextProcessingElementType.TextEmojiElement)
                {
                    // 어디까지 이모지 인거니?
                    int endIndex = i; 
                    GetRangeEmoji(textProcessingArray, i, out endIndex);
                    i = endIndex;
                    isEmoji = true;
                }
                ...
                else
                {
                    ...
                    int startIndex = textProcessingArray[backupIndex].stringIndex;
                    string emoji = text.Substring(startIndex, textProcessingArray[i].stringIndex + textProcessingArray[i].length - startIndex);
                    
                    // unicode를 hash로 속이자.
                    unicode = (uint)TMP_TextUtilities.GetSimpleHashCode(emoji);
                    character = GetEmojiTextElement(emoji);
                    // spriteAsset에서 가져오지 말고 그냥 쌩으로 해야겟다.
                }
                ...
                if (isEmoji)
                {
                    m_textInfo.characterInfo[m_totalCharacterCount].index = textProcessingArray[backupIndex].stringIndex;
                    int length = 0;
                    for(var s = backupIndex; s<= i; ++s)
                    {
                        length += textProcessingArray[s].length;
                    }
                    m_textInfo.characterInfo[m_totalCharacterCount].stringLength = length;
                }
                else
                {
        ...
        protected virtual void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_TextProcessingArray.Length && m_TextProcessingArray[i].unicode != 0; i++)
            {
                ...
                // FESTA-20 swpark, iOS 이모지 개발 
                if (m_TextProcessingArray[i].elementType == TextProcessingElementType.TextEmojiElement)
                {
                    // 어디까지 이모지 인거니?
                    int endIndex = i;
                    GetRangeEmoji(m_TextProcessingArray, i, out endIndex);
                    i = endIndex;
                    isEmoji = true;
                }
