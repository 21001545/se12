...
-(void) createDB {
    ...
    if([fileManager fileExistsAtPath: self.databasePath] == NO) {
        ...
        if(openResult == SQLITE_OK) {
            char *errMsg;
            const char *sql_stmt = "CREATE TABLE location (time REAL PRIMARY KEY, latitude REAL, longitude REAL, h_accuracy REAL, v_accuracy REAL);";
...
-(void) insertLocationAt:(double)time latitude:(double)latitude longitude:(double)longitude
              h_accuracy:(double)h_accuracy v_accuracy:(double)v_accuracy
{
    ...
    if(openResult == SQLITE_OK) {
        NSString *sql = [NSString stringWithFormat:
                         @"INSERT INTO location (time, latitude, longitude, h_accuracy, v_accuracy) VALUES(\"%f\", \"%f\", \"%f\", \"%f\", \"%f\")",
                         time, latitude, longitude, h_accuracy, v_accuracy ];
...
-(NSArray*) selectLocationsAfter: (double) time {
    ...
    if(openResult == SQLITE_OK) {
        NSString *sql = [NSString stringWithFormat:
                         @"SELECT time, latitude, longitude,h_accuracy,v_accuracy FROM location WHERE time > %f", time];
        ...
        if(prepare == SQLITE_OK) {
            while(sqlite3_step(statement) == SQLITE_ROW) {
                ...
                double h_accuracy = sqlite3_column_double(statement, 3);
                double v_accuracy = sqlite3_column_double(statement, 4);
                ...
                NSMutableDictionary *rowData = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                ...
                                                [NSNumber numberWithDouble:v_accuracy],
                                                    @"v_accuracy",
                                                nil];
